...
summary(dat$Age)        # Verify (adult -> Age >=18)
summary(dat$Income)     # Verify income (earners -> income > 0)

# Create new dataset with individuals who ear $150K or less
dat2 <- dat %>% filter(Income > 0 & Income <= 150000)
...
```{r, message=FALSE, echo=TRUE}
# Histogram
dat2 %>% ggplot(aes(x=Income)) + geom_histogram(bins=100) + labs(title = 'Distribution of Income')   
# Boxplot
dat2 %>% ggplot(aes(y=Income)) + geom_boxplot()+ coord_flip() + labs(title = 'Distribution of Income')     

library(table1)
table1(~Income, data=dat2)
```
<br>
...
```{r, message=FALSE, echo=TRUE}
dat2 %>% ggplot(aes(y=Income,x=Sex)) + geom_boxplot() + coord_flip() + labs(title = 'Income Distributions Conditional on Sex')
table1(~Income|Sex,  data = dat2)  
```
<br>
...
# dat2 %>% ggplot(aes(y=Income,x=Education))+geom_boxplot()+coord_flip()

# Reorder education levels so trend is more obvious
dat2$Education <- factor(dat2$Education, levels = c('Less than HS','HS Graduate',
                                                    'Some College/Associates', 'Bachelors',
                                                    'Masters', 'PhD/MD/JD'))
dat2 %>% ggplot(aes(y=Income,x=Education))+geom_boxplot()+coord_flip() + labs(title = 'Income Distributions Conditional on Level of Education')
``` 
<br>
...
```{r, message=FALSE, warning=FALSE, echo=TRUE}
table1(~Sex|Education, data = dat2)
```
<br>
...
```{r, message=FALSE, warning=FALSE, echo=TRUE}
ggplot(dat2,aes(y=Income,x=Education,fill=Sex)) + geom_boxplot()  +coord_flip() + labs(title = 'Income Distributions Conditional on Education \n for Males vs Females')
# table1(~Income|Sex*Education,  data = dat2)
``` 
<br>
...
*A major limitation of this study is that it does not allow us to account for several potentially confounding variables like type of job and race. In future work we could create statistical models that allow us to account for these variables.*
<br>
<br>
