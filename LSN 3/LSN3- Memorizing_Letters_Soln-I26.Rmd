...
library(tidyverse)
memory.dat <- read_csv('C:/Users/annyclaude.joseph/Desktop/MA376/MemoryDataI26.csv')
```
...
`r colorize("*Given that students were not randomly selected to participate in the study, this is a convenience sample. This sample could reasonably represent all cows in the Corps of Cadets. (Arguments could be made against.)*", "blue")`
...
`r colorize("*The distribution skews to the left. The average score was 12.3 letters with a standard deviation of 6.78.*  ", "blue")`
...
`r colorize("*Predicted score = 12.3 letters; SE of residuals = 6.78 letters.*", "blue")`
...
`r colorize("*The group with the meaningless sequence had a much lower mean than the group with a meaningful sequence (8.1 letters vs 16.0 letters). The standard deviation was also lower for the meaningless sequence (3.91 letters vs 6.76 letters).*", "blue")`
...
\begin{array}{ll}
      16.0 \,\, letters, & if \,\, meaningful \,\, sequence \\
      8.13 \,\, letters, & if\,\,  meaningless \,\, sequence
\end{array} 
\right.  $$

`r colorize("*SE of residuals = 5.62 letters.*", "blue")`
...
`r colorize("*The standard error of residuals is a little bit smaller, but the reduction amounts to about one point. *", "blue")`
...
`r colorize("*Possibly; the difference in sleep means is 0.95 hours or about 57 minutes.*", "blue")`
