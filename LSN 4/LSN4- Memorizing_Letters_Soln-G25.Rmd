...
library(tidyverse)
memory.dat <- read_csv('C:/Users/annyclaude.joseph/Desktop/MA376/MemoryDataG25.csv')[-1,]
# For the purpose of the exercise, we remove 1 student with meaningless sequence to
...
$$\small{\text{Predicted score = }\hat{y}_i = 14.3 \text{ letters;  SE of residuals = 4.98 letters.}}$$ 
...
\begin{array}{ll}
      18.5 \text{ letters, } &  \text{if meaningful sequence} \\
      10.1 \text{ letters, } &  \text{if meaningless sequence}
\end{array} 
\right.  $$

*SE of residuals = 2.55 letters.*
...
$$\small{\text{Predicted Score}} =  \hat{y}_i = 14.3 + \left\{
\begin{array}{ll}
      4.19, \text{ letters, } &  \text{if meaningful sequence} \\
      -4.19, \text{ letters, } &  \text{if meaningless sequence}
\end{array} 
\right.  $$
...
```{r}
diff <- memory.dat$Grpmeans[2] - memory.dat$Grpmeans[1]
